...
import { WasteAPI, SyncAPI } from "./api.js";
...
class WasteService {
    ...
    async load(date = today()) { const data = await WasteAPI.getByDate(date); this.records = new Map(data.map(x => [x.id, x])); return data; }
    async save(item) { const data = this.normalize(item); if (network.isOffline()) { await SyncAPI.enqueue("SAVE_WASTE", data); this.records.set(data.id, { ...data, _pendingSync: true }); return data; } await WasteAPI.save(data); this.records.set(data.id, data); return data; }
    async remove(id) { if (network.isOffline()) await SyncAPI.enqueue("DELETE_WASTE", { id }); else await WasteAPI.remove(id); this.records.delete(id); }
    ...
    byReason() { const m = {}; this.all().forEach(x => { const key = x.reason || "Unspecified"; m[key] = (m[key] || 0) + Number(x.quantity || 0); }); return Object.entries(m).map(([reason, quantity]) => ({ reason, quantity })).sort((a,b) => b.quantity-a.quantity); }
