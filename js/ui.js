...
class UI {
    ...
    resolveView(name) {
        ...
        const selectors = [`#${name}`, `#${name}-view`, `#${name}-screen`, `[data-view="${name}"]`];
        const element = selectors.map(selector => this.$(selector)).find(Boolean) || null;
        if (element) this.views[name] = element;
        return element;
    }
    ...
    show(selector) { this.$(selector)?.classList.remove("hidden"); }
    hide(selector) { this.$(selector)?.classList.add("hidden"); }
    toggle(selector) { this.$(selector)?.classList.toggle("hidden"); }
    ...
    loading(selector, state = true) {
        const el = this.$(selector); if (!el) return;
        if (state) { el.dataset.originalText = el.innerHTML; el.disabled = true; el.innerHTML = `<span class="spinner-border spinner-border-sm me-2"></span>Loading...`; }
        else { el.disabled = false; if (el.dataset.originalText) el.innerHTML = el.dataset.originalText; }
    }

    toast(message, type = "primary") {
        const container = document.getElementById("toast-container"); if (!container) return;
        const toast = document.createElement("div"); toast.className = `toast align-items-center text-bg-${type} border-0 show mb-2`;
        toast.innerHTML = `<div class="d-flex"><div class="toast-body"></div><button type="button" class="btn-close btn-close-white me-2 m-auto"></button></div>`;
        toast.querySelector(".toast-body").textContent = message;
        toast.querySelector("button").addEventListener("click", () => toast.remove());
        container.appendChild(toast); this.toasts.push(toast);
        setTimeout(() => { toast.remove(); this.toasts = this.toasts.filter(t => t !== toast); }, 3000);
    }
